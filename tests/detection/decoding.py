...
_, result = cv2.threshold(blur, 100, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
for i, temp in enumerate(stats):
    ...
    # width_left : 왼쪽
    while True:
        if (blur[y1][x1 - width_left] <= blur[y1][x1 - width_left + 1]) and (
            blur[y1][x1 - width_left + 1] > 60
        ):
            width_left += 1
            print(
                "왼쪽 좌표[x:{}][y:{}]:화소값{}, 좌표[y:{}][x:{}]:화소값:{}".format(
                    x1 - width_left,
                    y1,
                    (blur[y1][x1 - width_left]),
                    x1 - width_left + 1,
                    y1,
                    (blur[y1][x1 - width_left + 1]),
                )
            )
        else:
    ...
    # width_right : 오른쪽
    while True:
        if (blur[y2][x2 + width_right] <= blur[y2][x2 + width_right - 1]) and (
            blur[y2][x2 + width_right - 1] > 60
        ):
            width_right += 1
            print(
                "오른쪽 좌표[x:{}][y:{}]:화소값{}, 좌표[y:{}][x:{}]:화소값:{}".format(
                    x2 + width_right,
                    y2,
                    (blur[y2][x2 + width_right]),
                    x2 + width_right + 1,
                    y2,
                    (blur[y2][x2 + width_right - 1]),
                )
            )
        else:
    ...
    # height_top : 위
    while True:
        if (blur[y3 - height_top][x3] <= blur[y3 - height_top + 1][x3]) and (
            blur[y3 - height_top + 1][x3] > 60
        ):
            height_top += 1
            print(
                "위쪽 좌표[x:{}][y:{}]:화소값{}, 좌표[y:{}][x:{}]:화소값:{}".format(
                    x3,
                    y3 - height_top,
                    (blur[y3 - height_top][x3]),
                    x3,
                    y3 - height_top + 1,
                    (blur[y3 - height_top + 1][x3]),
                )
            )
        else:
            ymin = y3 - height_top
    ...
    # height_bottem : 아래
    while True:
        if (blur[y4 + height_bottem][x4] <= blur[y4 + height_bottem - 1][x4]) and (
            blur[y4 + height_bottem - 1][x4] > 60
        ):
            height_bottem += 1
            print(
                "아래쪽 좌표[x:{}][y:{}]:화소값{}, 좌표[y:{}][x:{}]:화소값:{}".format(
                    x4,
                    y4 + height_bottem,
                    (blur[y4 + height_bottem][x4]),
                    x4,
                    y4 + height_bottem - 1,
                    (blur[y4 + height_bottem - 1][x4]),
                )
            )
        else:
