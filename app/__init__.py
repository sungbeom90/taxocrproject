...
def dashboard():  # 매개변수 받아야하나? ㄴㄴ
    ...
    # 도넛 그래프
    labels, data = om.doughnutGraph(year_)

    # 라인 그래프
    data2 = om.lineGraph(year_)
...
def check_done():
    ...
    t_bill = (
        ...
        om.cost_replace(args_dict["b_cost_sup"]),
        om.cost_replace(args_dict["b_cost_tax"]),
        om.cost_replace(args_dict["b_cost_cash"]),
        om.cost_replace(args_dict["b_cost_check"]),
        om.cost_replace(args_dict["b_cost_note"]),
        om.cost_replace(args_dict["b_cost_credit"]),
        args_dict["p_id"],
    )
